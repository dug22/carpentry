...
public class IntegerColumnTest {
    ...
    private final IntegerColumn column = IntegerColumn.create("Test");
    ...
    public void appendTest() {
        ...
        column.appendAll(new Integer[]{3, 4});
    ...
    public void copyEmptyTest() {
        IntegerColumn emptyCopy = column.copyEmpty();
        assertTrue(emptyCopy.isEmpty());
    }

    @Test
    public void copyTest() {
        IntegerColumn copy = column.copy();
    ...
    public void uniqueTest() {
        column.append(10);
        IntegerColumn unique = column.unique();
        assertEquals(4, unique.size());
    }

    @Test
    public void filterTest() {
        IntegerColumn filter = column.filter(a -> a > 10);
    ...
    public void applyTest() {
        IntegerColumn apply = column.apply(a -> a * 2);
        assertEquals(8, apply.get(0));
        assertEquals(20, apply.get(1));
        assertEquals(16, apply.get(2));
        assertEquals(26, apply.get(3));
    }

    @Test
    public void sortTest() {
        column.sortAscending();
        assertArrayEquals(new Integer[]{4, 8, 10, 13}, column.getValues());
        column.sortDescending();
        assertArrayEquals(new Integer[]{13, 10, 8, 4}, column.getValues());
    }

    @Test
    public void replaceTest() {
        column.replace(new Integer[]{10, 8}, 15);
    ...
    public void numericMapTest() {
        IntegerColumn addition = column.plus(otherColumn()).asIntegerColumn();
        Integer[] additionResults = new Integer[]{14, 22, 17, 20};
        for (int i = 0; i < addition.size(); i++) {
            assertEquals(additionResults[i], addition.get(i));
        }

        IntegerColumn subtraction = column.minus(otherColumn()).asIntegerColumn();
        Integer[] subtractionResults = new Integer[]{-6, -2, -1, 6};
        for (int i = 0; i < subtraction.size(); i++) {
            assertEquals(subtractionResults[i], subtraction.get(i));
        }

        IntegerColumn product = column.times(otherColumn()).asIntegerColumn();
        Integer[] productResults = new Integer[]{40, 120, 72, 91};
        for (int i = 0; i < product.size(); i++) {
            assertEquals(productResults[i], product.get(i));
        }

        IntegerColumn division = column.divide(otherColumn()).asIntegerColumn();
        Integer[] divisionResults = new Integer[]{0, 0, 0, 1};
    ...
    private IntegerColumn otherColumn() {
        return IntegerColumn.create("Test 2", new Integer[]{10, 12, 9, 7});
    }
}
